...
class get_fvcom():
    ...
    def nearest_point(self, lon, lat, lons, lats, length):  #0.3/5==0.06
        ...
        for a in np.arange(12):
            c=np.argmin(distancelist)
            dis.append(distancelist[c])
            ins.append(insidep[c])
            distancelist=np.delete(distancelist,c)
        ...
        return ins,diss,dissum
    def nearest_point2(self, lon, lat, lons, lats, length):  #0.3/5==0.06
        '''Find the nearest point to (lon,lat) from (lons,lats),
           return the nearest-point (lon,lat)
           author: Bingwei'''
        p = Path.circle((lon,lat),radius=length)
    ...
    def get_url(self, starttime, endtime):
        ...
        if self.modelname == "GOM3":
            ...
            self.url = url
#http://www.smast.umassd.edu:8080/thredds/dodsC/fvcom/hindcasts/30yr_gom3?h[0:1:48450],u[0:1:333551][0:1:44][0:1:90414],v[0:1:333551][0:1:44][0:1:90414],
#temp[0:1:333551][0:1:44][0:1:48450],salinity[0:1:333551][0:1:44][0:1:48450],uwind_stress[0:1:333551][0:1:90414],vwind_stress[0:1:333551][0:1:90414]
            ...
            temp[{0}:1:{1}][0:1:44][0:1:48450],salinity[{0}:1:{1}][0:1:44][0:1:48450],
            uwind_stress[{0}:1:{1}][0:1:90414],vwind_stress[{0}:{1}][0:1:90414]'''
    ...
    def get_data(self,url):
        ...
        '''
        self.data = get_nc_data(url,'lat','lon','latc','lonc','siglay','h','nbe','u','v','zeta','temp','salinity','uwind_stress','vwind_stress')#,'nv'
        ...
        self.temp=self.data['temp'];self.salinity=self.data['salinity'];
        self.uwind_stress=self.data['uwind_stress'];self.vwind_stress=self.data['vwind_stress']
    ...
    def get_track(self,lon,lat,depth,starttime,wind,wind_get_type): #,b_index,nvdepth, 
        ...
        lonk,latk = self.shrink_data(lon,lat,self.lons,self.lats,4)#1 day nodes(red)
        
        try:
            if self.modelname == "GOM3" or self.modelname == "30yr":
                point1,dis1,dissum1 = self.nearest_point(lon, lat, lonl, latl,1)#elements(blue)
                point2,dis2,dissum2= self.nearest_point2(lon,lat,lonk,latk,4)#nodes(red)
            ...
            elementindex=[]
            nodeindex=[]
            for a in np.arange(len(point1)):
                #print 'point[a][0]',point1[a][0]
                index1=(np.where(self.lonc==point1[a][0]))
                index2=(np.where(self.latc==point1[a][1]))
                #print 'index1',index1[0][0]
                #print 'index2',index2[0][0]
                elementindex.append(np.intersect1d(index1[0][0],index2[0][0]))#nearest index elements(blue)
            #print elementindex
            for b in np.arange(len(point2)):
                #print 'point[b][0]',point2[b][0]
                index3=(np.where(self.lons==point2[b][0]))
                index4=(np.where(self.lats==point2[b][1]))
                #print 'index3',index3[0][0]
                #print 'index4',index4[0][0]
                nodeindex.append(np.intersect1d(index3[0][0],index4[0][0]))#nearest index nodes(red)
            #print nodeindex
            elementindex1=[]
            nodeindex1=[]
            dis11=[]
            ...
            point22=[]
            point11=[]
            for a in np.arange(len(elementindex)):
                if elementindex[a]<1000000000000000000:
                    elementindex1.append(elementindex[a][0])
                    dis11.append(dis1[a])
                    point11.append(point1[a])
            for a in np.arange(len(nodeindex)):
                if nodeindex[a]<1111111111111111111:
                    nodeindex1.append(nodeindex[a])
                    dis22.append(dis2[a])
                    point22.append(point2[a])
            h1=[]
            temp1=[]
            salt1=[]
            pa = self.eline_path(lon,lat)# boundary 
            waterdepth=[]
            #print 'elementindex1,nodeindex1',len(elementindex1),len(nodeindex1)
            if self.modelname == "30yr":
                for a in np.arange(len(nodeindex1)):
                    waterdepth.append(self.h[nodeindex1[a][0]])
            else:
                waterdepth = self.h[nodeindex]+self.zeta[0,nodeindex]
                '''
            waterde=[]
            for c in np.arange(len(waterdepth)):
                waterde.append(waterdepth[c]*dis2[c])
            wd=np.sum(waterde)/dissum2
            '''
            modpts['time'].append(self.mTime[0])
            depth_total=[]
            for a in np.arange(len(nodeindex1)):
                
                depth_total.append(self.siglay[:,nodeindex1[a]]*waterdepth[a])#; #print 'Here one'
                h1.append(self.h[nodeindex1[a]])
                
            de=[]
            for b in np.arange(len(depth_total)):
                de.append(depth_total[b]*dis2[b])
            
            depth_total1=np.sum(de)/dissum2
            layer = np.argmin(abs(depth_total1+depth)); #print 'layer',layer
            #print 'layer',layer
            modpts['layer'].append(layer); 
            #print 444444444444444444444
            #print elementindex1
            #print 'len(elementindex1)',len(elementindex1)
            #print 'len(elementindex)',len(elementindex)
            #print 'len(nodeindex1)',len(nodeindex1)
            #print 'len(nodeindex)',len(nodeindex)
            #print 'waterdepth',waterdepth
            #print 'depth',depth
            for a in np.arange(len(waterdepth)): 
                if waterdepth[a]<(abs(depth)): 
        ...
        for i in xrange(t):  
            ...
            if i!=0 and i%24==0 :
                ...
                lonk,latk = self.shrink_data(lon,lat,self.lons,self.lats,4)
            ...
            tt1=[]
            ss1=[]
            for cc in np.arange(len(nodeindex1)):
                temp1.append(self.temp[i,layer,nodeindex1[cc]])
                salt1.append(self.salinity[i,layer,nodeindex1[cc]])
            #print 'point2[0]',type(np.array(point22).T[0])
            #print 'len(h1)',len(h1)
            #print 'len(tem)',len(temp1)
            #print 'len(salt1)',len(salt1)
            p10=[]
            p11=[]
            
            for a in np.arange(len(np.array(point11).T[0])):
                p10.append(np.array(point11).T[0][a])
                p11.append(np.array(point11).T[1][a])
            p100,p111=np.meshgrid(p10,p11)
            #print p100,p111
            #print 'changdu',len(np.array(point22).T[0])
            hh=interpolate.griddata(np.array(point22),h1,(p100,p111), method='cubic')
            
            tt=interpolate.griddata(np.array(point22),temp1,(p100,p111),method='cubic')
            ss=interpolate.griddata(np.array(point22),salt1,(p100,p111),method='cubic')
            
            for a in np.arange(len(p111)):
                hh1.append(hh[a][a])
                tt1.append(tt[a][a])
                ss1.append(ss[a][a])
            print 'hh1',hh1
            print 'tt1',tt1
            print 'ss1',ss1
            for a in np.arange(len(elementindex1)):
                #print 'elementindex1[a]',elementindex1[a]
                #print 'self.u[i,layer,elementindex1[a]][0]',self.u[i,layer,elementindex1[a]]
                u_t1.append( self.u[i,layer,elementindex1[a]]); v_t1.append(self.v[i,layer,elementindex1[a]])
                u_stress.append(self.uwind_stress[i,elementindex1[a]]);v_stress.append(self.vwind_stress[i,elementindex1[a]])
                #print 6666666666
                u_t2.append(self.u[i+1,layer,elementindex1[a]]); v_t2.append(self.v[i+1,layer,elementindex1[a]])
            #samnum=len(elementindex1)
            #testsamnum=samnum
            #forcastsamnum=1
            uus2=[]
            vvs2=[]
            ...
            v_t11=[]
            dis12=[]
            for b in np.arange(len(u_stress)):
                if tt1[b][0]<100000:
                    u_stress1.append(u_stress[b])
                    v_stress1.append(v_stress[b])
                    dis12.append(dis11[b])
                    #dis11.append
                    hh11.append(hh1[b])
                    tt11.append(tt1[b])
                    ss11.append(ss1[b])
            ...
            dis12=dis12/np.sum(dis12)
            #print 'dis12',dis12
            uus2.append(interpolate.griddata(np.array(point11),u_stress,(lon,lat),method='cubic'))
            vvs2.append(interpolate.griddata(np.array(point11),v_stress,(lon,lat),method='cubic'))
            #uus2.append(us2[0][0])
            #vvs2.append(vs2[0][0])
            if uus2>100000000000000000000000000:
                uus2=[]
                vvs2=[]
                uus2.append((np.sum(np.array(u_stress)*np.array(dis11)))/np.sum(dis11))
                vvs2.append((np.sum(np.array(v_stress)*np.array(dis11)))/np.sum(dis11))
                #hh2.append((np.sum(np.array(h11)*np.array(dis22)))/np.sum(dis22))
            print 'uus2',uus2
            print 'vvs2',vvs2
            hiddenunitnum=8
            indim=5
            ...
            noise = 0.0000001*np.random.rand(sampleoutnorm.shape[0],sampleoutnorm.shape[1])
            sampleoutnorm += noise
            
            maxepochs = 100000
            learnrate = 0.035
            errorfinal = 0.00000001
            ...
            ss2=[]
            tt2=[]
            hh2.append(interpolate.griddata(np.array(point22),h1,(lon,lat), method='cubic'))
            
            tt2.append(interpolate.griddata(np.array(point22),temp1,(lon,lat),method='cubic'))
            ss2.append(interpolate.griddata(np.array(point22),salt1,(lon,lat),method='cubic'))
            ...
            temp11=[]
            salt11=[]
            for a in np.arange(len(h1)):
                h11.append(h1[a][0])
                temp11.append(temp1[a][0])
                salt11.append(salt1[a][0])
            if tt2>1000000000000000:
                tt2=[]
                ss2=[]
                hh2=[]
                tt2.append((np.sum(np.array(temp11)*np.array(dis22)))/np.sum(dis22))
                ss2.append((np.sum(np.array(salt11)*np.array(dis22)))/np.sum(dis22))
                hh2.append((np.sum(np.array(h11)*np.array(dis22)))/np.sum(dis22))
            print tt2
            newsamplein = np.mat([uus2,vvs2,hh2,tt2,ss2])
            print 'newsamplein',newsamplein
            newsampleinnorm = (2*(np.array(newsamplein[0].T)-sampleinminmax.transpose()[0])/(sampleinminmax.transpose()[1]-sampleinminmax.transpose()[0])-1).transpose()
            print 'newsampleinnorm',newsampleinnorm
            print 333333333333333333333333
            newhiddenout = logsig((np.dot(w1,newsampleinnorm).transpose()+b1.transpose())).transpose()
            newnetworkout = (np.dot(w2,newhiddenout).transpose()+b2.transpose()).transpose()
            OUT=((newnetworkout.transpose()+1)*1./2*(sampleoutminmax.transpose()[1]-sampleoutminmax.transpose()[0])+sampleoutminmax.transpose()[0]).T
            print 'OUT',OUT
            ...
            print 'uu',uu
            print 'vv',vv
            if uu>1000000000000000000000 or uu<=0 :
                u_t=[]
                v_t=[]
                #print 555555555555555555555
                for a in np.arange(len(u_t1)):
                    
                    u_t.append((u_t1[a]+u_t2[a])/2)#;
                    v_t.append((v_t1[a]+v_t2[a])/2)
                ...
                uu=0
                vv=0
                for a in np.arange(len(u_t)):
                    u1.append(u_t[a]*dis11[a])
                    v1.append(v_t[a]*dis11[a])
            #print 'np.sum(u1)',np.sum(u1),'np.sum(dis11)',dis11
            #print 'np.sum(v1)',np.sum(v1)
                uu=(np.sum(u1))/np.sum(dis11)
                vv=(np.sum(v1))/np.sum(dis11)
            print 'min',np.min(np.array(point2).T[0])
            print 'max',np.max(np.array(point2).T[0])
            print 'min',np.min(np.array(point2).T[1])
            print 'max',np.max(np.array(point2).T[1])
            print lon,lat
#
#            u_t=[]
#            v_t=[]
...
#                v1.append(v_t[a]*dis11[a])
            #print 'np.sum(u1)',np.sum(u1),'np.sum(dis11)',dis11
            #print 'np.sum(v1)',np.sum(v1)
#            uu=(np.sum(u1))/np.sum(dis11)#+fu(lon,lat)[0]
#            vv=(np.sum(v1))/np.sum(dis11)#+fv(lon,lat)[0]
            starttimes=starttime+timedelta(hours=i)
            #print 'uu',uu,'vv',vv
            #print 'starttime',starttimes
            if wind==0:

                dx = 60*60*uu; dy = 60*60*vv
            else:
                ...
                dx = 60*60*uu+60*60*u_wind[0][0][0]*wind; dy = 60*60*vv+60*60*v_wind[0][0][0]*wind
            ...
            #print 'temlon',temlon,'temlat',temlat
