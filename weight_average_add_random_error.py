    ...
    def get_track(self,lon,lat,depth,starttime,wind,wind_get_type): #,b_index,nvdepth, 
        ...
        try:
            if self.modelname == "GOM3" or self.modelname == "30yr":
                ...
                point2,dis2,dissum2= self.nearest_point(lon,lat,lonk,latk,1)#nodes(red)
        ...
        for i in xrange(t):  
            ...
            uu=(np.sum(u1))/np.sum(dis11)-0.0035327735964#+fu(lon,lat)[0]
            vv=(np.sum(v1))/np.sum(dis11)-0.00114320186387#+fv(lon,lat)[0]
            ...
            try:
                if self.modelname == "GOM3" or self.modelname == "30yr":
                    point1,dis1,dissum1 = self.nearest_point(lon, lat, lonl, latl,1)
                    point2,dis2,dissum2= self.nearest_point(lon,lat,lonk,latk,1.2)
